...
class Number(ABC):
    ...
    def __add__(self, anotherNumber:Number) -> Number:
        ...
        digitsOfNumber2:np.ndarray = self.__getValuesOf(anotherNumber.getValue(), maxLength)
        indexOfCarry:int = 0
    ...
    def __sub__(self, substrahend:Number) -> Number:
        ...
        digitsOfNumber1:np.ndarray = self.__getValuesOf(self.getValue(), maxLength)
        digitsOfNumber2:np.ndarray = self.__getValuesOf(substrahend.getValue(), maxLength)
            ...
            if (indexOfDigit1 < indexOfDigit2):
                for i in range(index-1, -maxLength-1, -1):
                    if (digitsOfNumber1[i] != "0"):
                        borrowSourcePositionInDigits:int = self.__getIndexOf(digitsOfNumber1[i])
                        digitsOfNumber1[i] = self.digits[borrowSourcePositionInDigits - 1]
                        additionalLap += 1
    ...
    def __floordiv__(self, anotherNumber: Number) -> Number:
        if not isinstance(anotherNumber, self.__class__):
            raise TypeError("Error: No se pueden dividir números de distintos sistemas numéricos")
