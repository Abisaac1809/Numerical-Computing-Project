...
class SolveEquations(FileProcess):
    def execute(self):
        ...
        print("\n\nArchivos disponibles:")
        valueFilePosition = self.dataValidator.chooseOptionOf(
            availableFiles, "Elige el archivo que contiene los valores: "
        )
    ...
    def calculateAndWriteResults(
        ...
        for i in range(len(equations)):
            for j in range(len(equations[i])):
                try:
                    if equations[i][j]:
                        result = self.basicEquationSolver.solve(
                    ...
                    errorMessage = (
                        "Error: División por cero indefinida: ∄c∈R tal que a=0xc."
                    )
                    self.fileWriter.writeEquationAndError(
                        filePath, equations[i][j], str(errorMessage)
                    )
