...
class SolveMatrixEquations(FileProcess):
    def execute(self):
        ...
        try:
            ...
            print(
                f"Lo sentimos, ha ocurrido un error que impide leer los archivos: {error}"
            )
            return

        print("\n\nArchivos disponibles:")
        equationsFilePosition: int = self.dataValidator.chooseOptionOf(
            availableFiles, "Elige el archivo que contiene las ecuaciones: "
        )
            ...
            for j in range(len(equations[0])):
                if equations[i][j]:
                    for char in equations[i][j]:
                        if (
                            char in availableVariableNames
                            and char not in variableNamesReaded
                        ):
    ...
    def determinateVariables(
        self, availableFiles: np.ndarray, variableNames: LinkedList
    ) -> dict:
        ...
        for i in range(variableNames.getSize()):
            ...
            print("\n\nArchivos disponibles:\n")
            matrixFilePosition = self.dataValidator.chooseOptionOf(
                availableFiles, f"Ingresa el archivo que contiene la matriz {name}: "
            )
    ...
    def calculateAndWriteResults(
        self, filePath: str, equations: np.ndarray, variables: dict
    ):
            ...
            for j in range(len(equations[i])):
                try:
                    if equations[i][j]:
                        ...
                        result = self.matrixEquationSolver.solve(
                            equations[i][j], variables
                        )
                        self.fileWriter.writeEquationAndResult(
                            filePath, equations[i][j], result
                        )
                except ImposibleMatrixOperation as error:
                    self.fileWriter.writeEquationAndError(
                        filePath, equations[i][j], str(error)
                    )
                    ...
                    self.fileWriter.writeEquationAndError(
                        filePath, equations[i][j], errorMessage
                    )
