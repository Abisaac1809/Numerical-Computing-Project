...
class SolveEquationSystem(FileProcess):
    def execute(self):
        ...
        try:
            ...
            print(
                f"Lo sentimos, ha ocurrido un error que impide realizar la operación: {error}"
            )
            return

        print("Archivos disponibles para escanear\n")
        filePosition = self.dataValidator.chooseOptionOf(
            availableFiles, "Ingresa el archivo que quieres escanear: "
        )
    ...
    def __solveSystemOfEquation(
        ...
        try:
            ...
            raise SystemDontHaveSolution(
                "Error: El sistema de ecuaciones no tiene una solución"
            )
