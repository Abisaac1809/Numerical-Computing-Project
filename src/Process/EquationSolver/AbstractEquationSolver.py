...
class AbstractEquationSolver(ABC):
    ...
    def _tokenize(self, expression: str) -> LinkedList:
        ...
        while i < len(expression):
            ...
            if char == " ":
            ...
            elif self._isOperator(char) or char in self.openBrackets or char in self.closeBrackets:
                if char in self.openBrackets and tokens.getSize() != 0 and (self._isNumber(tokens.get(-1)) or self._isVariable(tokens.get(-1))):
                    tokens.addLast('*') 
                tokens.addLast(char)
                i += 1
                
            elif self._isNumber(char) or (char == '.' and i + 1 < len(expression) and self._isNumber(expression[i+1])):
                num_str = ""
                while i < len(expression) and (self._isNumber(expression[i]) or expression[i] == '.'):
                ...
                if tokens.getSize() != 0 and (self._isNumber(tokens.get(-1)) or self._isVariable(tokens.get(-1))):
                    tokens.addLast('*')
    ...
    def _shuntingYard(self, tokens: LinkedList) -> Queue:
        ...
        for i in range(tokens.getSize()):
            ...
            if self._isNumber(token):
                ...
                while (operatorStack.getSize() > 0 and operatorStack.showStack() in self.operators 
                        and self._getOperatorPrecedence(operatorStack.showStack()) >= self._getOperatorPrecedence(token)):
    ...
    def _evaluatePostfix(self, postfixNotation: Queue, variables: dict) -> float:
        if not isinstance(postfixNotation, Queue) or not isinstance(variables, dict):
            raise ValueError("Error: Has ingresado una pila inválida o un diccionario inválido")
